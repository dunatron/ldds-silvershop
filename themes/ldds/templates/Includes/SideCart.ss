        ...
        <% with $Cart %>
            ...
            <% loop $Items %>
                <div class="item $EvenOdd $FirstLast">
                    <% if $Product.Image %>
                        <div class="image">
                            <a href="$Product.Link" title="<%t Shop.ReadMoreTitle "Click here to read more on &quot;{Title}&quot;" Title=$Title %>">
                                <% with $Product %>$Image.FocusFill(125, 80)<% end_with %>
                            </a>
                        </div>
                    <% end_if %>
                    <p class="title">
                    ...
                    <a class="remove ajax" href="$removeallLink" title="<%t ShoppingCart.RemoveTitle "Remove &quot;{Title}&quot; from your cart." Title=$TableTitle %>">x</a>
                </div>
            <% end_loop %>
        <% end_with %>
    <% else %>
